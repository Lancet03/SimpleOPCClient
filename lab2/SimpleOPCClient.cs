...
namespace lab2
{
    ...
    internal class SimpleOPCClient
    {
        ...
        private List<string> BrowseOPCObject(ReferenceDescription rd, int level)
        {
            ...
            string strPadding = new String(' ', 2 * level); foreach (var nextRd in nextRefs)
            {
                switch (nextRd.NodeClass)
                {
                    case NodeClass.Object:
                        retVal.Add(strPadding + string.Format("+ {0}: {1}", nextRd.DisplayName, nextRd.NodeClass));
                    ...
                    case NodeClass.Variable:
                        ...
                        retVal.Add(strPadding + string.Format("+ {0}: {1}, {2}", nextRd.DisplayName, nextRd.NodeClass, strValueType));
                        break;
                }
            }
            return retVal;
        ...
        {
            if ((m_session == null) || (ConnectionStatus == false)) return null;
            NodeId nodeId = new NodeId(strNodeId);
            ...
            MonitoredItem item = sub.MonitoredItems.FirstOrDefault((mi) => mi.StartNodeId == nodeId);
            if (item == null)
            {
                ...
                sub.AddItem(monitoredItem); sub.ApplyChanges();
                OPCUnsubscribeObject unsub = new OPCUnsubscribeObject(monitoredItem); return unsub;
            }
            return null;
        }
        ...
        private void MonitoredItem_Notification(MonitoredItem monitoredItem, MonitoredItemNotificationEventArgs e)
        {
            ...
            {
                Debug.WriteLine(monitoredItem.StartNodeId.ToString() + ": " + itemNotification.Value.ToString());
                if (m_NodeValueChanged != null)
                {
                    m_NodeValueChanged(this, new NodeValueChangedEventArgs(monitoredItem.StartNodeId.ToString(), itemNotification.Value.ToString()));
                }
            }
        }
        ...
        public class OPCUnsubscribeObject : IDisposable
        {
            ...
            public OPCUnsubscribeObject(MonitoredItem item) { this.item = item; }
            public void Dispose() { Unsubscribe(); }
            private void Unsubscribe()
            {
                if ((item != null) && (item.Subscription != null))
                {
                    var sub = item.Subscription; sub.RemoveItem(item); sub.ApplyChanges();
